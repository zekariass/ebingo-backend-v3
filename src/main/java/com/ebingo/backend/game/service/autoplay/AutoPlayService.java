...
//
//    // Tunables - OPTIMIZED for 50 agents × 3 rooms × 100 bots = 15K concurrent bots
//    private static final long JOIN_DELAY_MIN_MS = 300;  // Reduced from 500ms for faster spawning
//    private static final long JOIN_DELAY_MAX_MS = 600;  // Reduced from 800ms
//    private static final Duration SPAWN_LOCK_TTL = Duration.ofSeconds(5);
//    private static final Duration BINGO_CLAIM_DELAY = Duration.ofMillis(2000);  // Reduced from 3000ms
//
//    // Concurrency controls - OPTIMIZED for high-scale multi-agent platform
//    private static final int JOIN_CONCURRENCY_SOFT_CAP = 20;     // Increased from 6 (3.3x) - faster game starts
//    private static final int MARK_CONCURRENCY_PER_ROOM = 128;    // Increased from 64 (2x) - handle 100 bots/room
//    private static final int GLOBAL_INFLIGHT_MARK_CALLS = 4096;  // Increased from 512 (8x) - support 150 concurrent rooms
//
 ...
 *
 * IMPORTANT FIX:
 * - numberIndex no longer copies lists on every append (O(N^2) bug removed).
 *   Uses ConcurrentLinkedQueue per number for lock-free appends.
 *
 * TUNED FOR:
 * - 2 vCPU / 8GB
 * - up to 10 rooms concurrently
 * - up to 100 bots per room
 * - 1 card per bot (~1000 bots total)
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class AutoPlayService {
    ...
    // Tunables
    private static final long JOIN_DELAY_MIN_MS = 300;
    ...
    private static final int MARK_CONCURRENCY_PER_ROOM = 16;    // was 128
    private static final int GLOBAL_INFLIGHT_MARK_CALLS = 256;  // was 4096
    ...
    private final class RoomRuntime {
        ...
        // ✅ FIX: per-game index: number -> refs
        // Using ConcurrentLinkedQueue removes list-copy O(N^2) bug during registerBot().
        private final Map<Integer, Queue<BotCardRef>> numberIndex = new ConcurrentHashMap<>();
        ...
        Mono<Void> onNumberDrawn(Long gameId, Integer drawnNumber) {
            ...
            Queue<BotCardRef> refs = numberIndex.get(drawnNumber);
            if (refs == null || refs.isEmpty()) return Mono.empty();
        ...
        private Mono<Void> registerBot(UserProfile botProfile, List<String> cardIds) {
            ...
            for (String cardId : cardIds) {
                ...
                for (Integer n : flattenCard(cardInfo.getNumbers())) {
                    ...
                    // ✅ FIX: O(1) append, no copying
                    numberIndex
                            .computeIfAbsent(n, k -> new ConcurrentLinkedQueue<>())
                            .add(new BotCardRef(botId, cardId));
                }
            }
